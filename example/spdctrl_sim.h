...
class SpdCtrlSim final:public Sim<_Controller>{
public:
public:
    SpdCtrlSim(_Controller* _k, double _time_step=1e-4);
    ~SpdCtrlSim();
...
template <class _Controller>
SpdCtrlSim<_Controller>::SpdCtrlSim(_Controller* _k, double _time_step)
...
template <class _Controller>
SpdCtrlSim<_Controller>::~SpdCtrlSim(){
    k_ = nullptr;
}

template <class _Controller>
auto SpdCtrlSim<_Controller>::setInput(const arma::mat& _in) -> void{
    u_ = _in;
}

template <class _Controller>
auto SpdCtrlSim<_Controller>::updateVariables() -> void{
    state_trans_ = arma::expmat(k_->A() * dt_);
}

template <class _Controller>
auto SpdCtrlSim<_Controller>::signalCalc() -> arma::mat{
...
template <class _Controller>
auto SpdCtrlSim<_Controller>::getSig() -> arma::mat{
    boost::mutex::scoped_lock lk(this->sig_mtx_);
    return plotted_sig_;
}

template <class _Controller>
auto SpdCtrlSim<_Controller>::getOutputSig() const -> arma::mat{
    boost::mutex::scoped_lock lk(this->sig_mtx_);
    return output_;
}

template <class _Controller>
auto SpdCtrlSim<_Controller>::propagate(const arma::mat &_in) -> arma::mat{
    setInput(_in);
