...
LowerLFT<_StateSpace, z2_size, w2_size, y2_size, u2_size>::LowerLFT(_StateSpace* _ss)
    ...
    B1_ = this->ss_->B().head_rows(w2_size);

    B2_ = this->ss_->B().tail_rows(u2_size);

    C1_ = this->ss_->C().head_cols(z2_size);
    C2_ = this->ss_->C().tail_cols(y2_size);

    D11_ = this->ss_->D().submat(0, 0, w2_size - 1, w2_size - 1);
    D12_ = this->ss_->D().submat(0, w2_size,
                                  w2_size - 1, w2_size + u2_size - 1);
    D21_ = this->ss_->D().submat(w2_size, 0,
                                  w2_size + y2_size - 1, w2_size - 1);
    D22_ = this->ss_->D().submat(w2_size, w2_size,
                                 w2_size + y2_size - 1, w2_size + u2_size - 1);
}
